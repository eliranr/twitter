...
export default function Widgets({newResults, randomUsersResults}) {
    ...
    <div className="xl:w-[600px] hidden lg:inline ml-8 space-y-5">
        ...
        <div className="text-gray-700 space-y-3 bg-gray-100 rounded-xl pt-2 w-[90%] xl:w-[75%]">
            <h4 className="font-bold text-xl px-4">What't Happening</h4>
            <AnimatePresence>
            ...
            <AnimatePresence>
                {randomUsersResults.slice(0, randomUserNum).map((randomUser) => (
                    <motion.div 
                        ...
                        exit={{opacity: 0}}    
                    >
                        <div key={randomUser.login.username} className='flex items-center hover:bg-gray-200 px-4 py-2 cursor-pointer transition duration-500 ease-ou'>
                            <img width='50' className="rounded-full mx-5" src={randomUser.picture.thumbnail} />
                            <div className="truncate ml-4 leading-5">
                                <h4 className="hover:underline font-bold text-[14px] truncate">{randomUser.login.username}</h4>
                                <h5 className="text-[13px] text-gray-500 truncate">{randomUser.name.first + ' ' + randomUser.name.last}</h5>
                            </div>
                            <button className="rounded-full bg-blue-400 px-3.5 py-1.5 ml-auto text-white text-sm font-bold">Follow</button>
                        </div>
                    </motion.div>
                ))}
            </AnimatePresence>
